---
name: roborock-challenge
description: Méta-audit GED de l'écosystème Claude — passe un coup de Roborock sur skills, agents, hooks, rules, memory, CLAUDE.md et tous les .md/.yml/.json de config du projet. Détecte fichiers orphelins, liens morts, frontmatters cassés, doublons sémantiques, refs fantômes, knowledge rot, et trous de cartographie/navigabilité (peut-on trouver l'info depuis n'importe où). Audit PUR (rapport seul, jamais de write). Invoquer quand l'utilisateur dit "Roborock", "passe le Roborock", "nettoie la GED", "y'a quoi qui sert plus", "audite ma connaissance Claude", "fichiers orphelins", "knowledge rot", "revoir les liens", "cartographie complète", "carte de navigation", ou en fin de grosse session avant un commit/déploiement structurel. Complémentaire au skill `cheikh` (qui couvre le code et le projet) — Roborock cible la connaissance, les process, le métier, l'organisation.
...
Voir [references/refresh-anthropic-doc.md](references/refresh-anthropic-doc.md) pour la liste exhaustive des URLs et la méthode de diff.

**Pourquoi** : les règles Anthropic (frontmatter SKILL.md, structure agents, conventions hooks, format memory) évoluent. Les `references/` de Roborock sont gravés à la création — sans refresh, l'audit utilise des règles potentiellement périmées.

**Méthode rapide** :
1. WebFetch sur 4 URLs canoniques (voir `references/refresh-anthropic-doc.md`)
2. Extraire les contraintes actuelles (limites chars, regex name, mots réservés, structure dossiers)
3. Comparer avec ce que disent les `references/checks-*.md` locaux
...
- Ne PAS modifier les `references/` sans validation explicite de l'utilisateur

---

## Phase 1 — Frontmatters & métadonnées

Voir [references/checks-frontmatters.md](references/checks-frontmatters.md) pour la grille détaillée.
...
Voir [references/checks-skills.md](references/checks-skills.md).
...
- Skills avec `references/` ou `scripts/` dont les fichiers cités sont absents

---

## Phase 3 — Agents

Voir [references/checks-agents.md](references/checks-agents.md).
...
Voir [references/checks-hooks-rules.md](references/checks-hooks-rules.md).
...
Voir [references/checks-claude-memory.md](references/checks-claude-memory.md).
...
Voir [references/checks-links.md](references/checks-links.md).
...
Voir [references/checks-orphans.md](references/checks-orphans.md).
...
Voir [references/report-template.md](references/report-template.md) pour le gabarit complet.
